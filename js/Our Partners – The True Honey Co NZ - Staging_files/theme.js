...
slate.Variants = (function() {
  ...
  Variants.prototype = _.assignIn({}, Variants.prototype, {
    ...
    _filterThumnails: function(variant) {
      
//       $('[data-thumbnail-size]').hide();
      var selected_size = variant.featured_image.alt;
      var thumbnail_selector = '[data-thumbnail-size="' + selected_size + '"]'
...
theme.MobileNav = (function() {
  ...
  function cacheSelectors() {
    cache = {
      ...
      $siteHeader: $('.site-header'),
       $siteHeaderMobile: $('.site-header__mobile-nav'), //adam: update background color on click
  ...
  function openMobileNav() {
    ...
    cache.$siteHeaderMobile.addClass('js-menu--is-open'); //adam: update background color on click

    //adam: remove page container transform
...
//     });
  ...
  function closeMobileNav() {
    ...
    setTimeout(function() {
      cache.$siteHeaderMobile.removeClass('js-menu--is-open'); //adam: fade background color on click
    }, 300);
...
$(document).ready(function(){
  ...
  // get for product page
    $.get("https://ipinfo.io/json", function (response) {
        //here you get details in response.
//         console.log(response.country);
        if(response.country == 'NZ'){
            var rawPrice = $('#uniquePrice span.price-item.price-item--regular').text();
            var unitPrice = rawPrice.replace('$', '');
            var updatedPrice = parseFloat(unitPrice * 1.15).toFixed(2);
          if (!isNaN(updatedPrice)){
            $('#uniquePrice span.price-item.price-item--regular').text('$'+updatedPrice);
          }
        }
    }, "jsonp");
  
  // get for collection page
  $.get("https://ipinfo.io/json", function (response) {
        //here you get details in response.
//         console.log(response.country);
        if(response.country == 'NZ'){
            $.each($('.uniquePrice'), function() {
                var rawPrice = $(this).find('span.price-item.price-item--regular').text();
                var unitPrice = rawPrice.replace('$', '');
                var updatedPrice = parseFloat(unitPrice * 1.15).toFixed(2);
              if (!isNaN(updatedPrice)){
                $(this).find('span.price-item.price-item--regular').text('$'+updatedPrice);
              }
            });
        }
}, "jsonp");
  
  // hide thumbnail of variant featured image
//   	var thumbnail_selector = "[data-thumbnail-size='400g']";
